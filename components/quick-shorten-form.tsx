...
export function QuickShortenForm({
  ...
  return (
    <form action={formAction} className="w-full">
      ...
      <div className="mt-3 flex min-w-0 flex-col gap-1.5 rounded-lg border border-[#e6e6e6] bg-white px-3.5 py-2 shadow-sm transition focus-within:border-[#cfcfcf] sm:h-9 sm:flex-row sm:items-center sm:gap-0 sm:py-0">
        <span className="min-w-0 shrink truncate text-left text-xs text-[#8a8a8a] sm:max-w-[48%]">
          {baseUrl}/
        </span>
        <input
          className="h-8 min-w-0 flex-1 bg-transparent text-xs text-[#222222] outline-none placeholder:text-[#a3a3a3] sm:h-full sm:px-1"
          name="customSlug"
